 ...
 */
showdown.helper.escapeCharacters = function escapeCharacters(
  text,
  charsToEscape,
  afterBackslash
) {
...
showdown.subParser("hashHTMLSpans", function (text, config, globals) {
  ...
  var matches = showdown.helper.matchRecursiveRegExp(
    text,
    "<code\\b[^>]*>",
    "</code>",
    "gi"
  ...
  for (var i = 0; i < matches.length; ++i) {
    text = text.replace(
      matches[i][0],
      "~L" + (globals.gHtmlSpans.push(matches[i][0]) - 1) + "L"
    );
  }
  return text;
});
